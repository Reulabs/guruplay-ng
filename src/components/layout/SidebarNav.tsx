...
export const SidebarNavItem = ({ icon: Icon, label, path, activeMatch }: SidebarNavItemData) => {
  const { pathname, search } = useLocation();
  const isActive = activeMatch ? activeMatch(pathname, search) : pathname === path;
  ...
  return (
    ...
    >
      <Icon className={cn('h-5 w-5 transition-colors', isActive ? 'text-black' : 'text-white/55 group-hover:text-white')} />
