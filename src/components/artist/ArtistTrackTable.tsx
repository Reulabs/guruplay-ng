...
const ArtistTrackTable = ({ tracks, onPlayTrack, title = 'Top Tracks' }: ArtistTrackTableProps) => {
  return (
    <section>
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="text-sm font-bold text-white/45 hover:text-white">View all</button>
      </div>
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035]">
        ...
        {tracks.map((track, index) => (
          ...
          >
            <span className="text-sm text-white/45 group-hover:hidden">{index + 1}</span>
            ...
            <span className="flex min-w-0 items-center gap-3">
              ...
              <span className="min-w-0">
                ...
                <Typography variant="caption" className="text-white/45" truncate>
                  {track.artist}
                </Typography>
              </span>
            </span>
            <Typography variant="body-sm" className="hidden text-white/55 md:block" truncate>
              {track.album}
            </Typography>
            <Typography variant="body-sm" className="hidden text-right text-white/55 md:block">
