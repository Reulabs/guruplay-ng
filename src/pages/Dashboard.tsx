...
import { Clock, Eye, Heart, Loader2, LogOut, Music, Music2, Plus, Settings, TrendingUp, Users } from "lucide-react";
...
const Dashboard = () => {
  ...
  const { user, isAuthenticated, isAuthLoading, logout, openAuthDialog } = useAuth();
  ...
  const isApprovedArtist = artistProfile?.user_type === UserType.Artist && artistProfile.approval_status === ArtistApprovalStatus.Approved;
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    if (!nextProfile || nextProfile.approval_status !== ArtistApprovalStatus.Approved || nextProfile.user_type !== UserType.Artist) {
      setUploadedSongs([]);
      setMetrics(emptyMetrics);
      setChartData([]);
      setLoading(false);
    ...
    const nextChartData = last7Days.map((date) => {
      ...
      return {
        ...
        listeners: rows.reduce((sum, row) => sum + (row.unique_listeners || 0), 0),
      };
    });

    const totalPlays = songs.reduce((sum, song) => sum + (song.total_plays || 0), 0);
    const totalLikes = songs.reduce((sum, song) => sum + (song.total_likes || 0), 0);
    const uniqueListeners = new Set((activityData || []).map((activity) => activity.user_id)).size;
    const avgListenTime = activityData && activityData.length > 0
      ? Math.round(activityData.reduce((sum, activity) => sum + (activity.listen_duration || 0), 0) / activityData.length)
      : 0;
  ...
  const applicationStatusCopy = useMemo(() => {
    ...
    if (artistProfile.approval_status === ArtistApprovalStatus.Pending) {
      return {
        ...
        description: 'Your artist profile has been submitted. Upload access unlocks after admin approval.',
      };
    }
    ...
    if (artistProfile.approval_status === ArtistApprovalStatus.Rejected) {
      return {
        ...
        description: 'Review your profile details and resubmit your application.',
      };
    }
  ...
  const renderUnauthenticated = () => (
    <div className="mx-auto max-w-3xl rounded-3xl border border-white/10 bg-white/[0.04] p-8 text-center">
      ...
      <Typography variant="body" tone="muted" className="mt-2">
        Apply as an artist, manage approvals, and upload tracks from one workspace.
      </Typography>
      <Button onClick={() => openAuthDialog('login')} className="mt-6 rounded-full px-6 font-bold">
        Log in
      </Button>
    </div>
  );
  ...
  const renderApplicationForm = () => (
    <div className="mx-auto max-w-3xl rounded-3xl border border-white/10 bg-white/[0.04] p-6 md:p-8">
      ...
      <Typography variant="body" tone="muted" className="mt-2">
        Approved artists can upload songs, manage catalog performance, and view listener analytics.
      </Typography>
      ...
      <form onSubmit={handleApply} className="mt-8 grid gap-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(event) => setFormData((current) => ({ ...current, artistName: event.target.value }))}
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(event) => setFormData((current) => ({ ...current, socialUrl: event.target.value }))}
            className="border-white/10 bg-white/[0.06]"
          />
        </div>
        <div className="space-y-2">
          ...
          <Textarea
            ...
            className="min-h-32 resize-none border-white/10 bg-white/[0.06]"
            required
          />
        </div>
        <Button type="submit" className="mt-2 w-fit rounded-full px-6 font-bold" disabled={isApplying}>
          {isApplying ? 'Submitting...' : artistProfile ? 'Resubmit application' : 'Submit application'}
        </Button>
      </form>
    </div>
  );
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard title="Total Plays" value={metrics.totalPlays.toLocaleString()} icon={TrendingUp} iconColor="text-primary" />
            <MetricCard title="Total Listeners" value={metrics.totalListeners.toLocaleString()} icon={Users} iconColor="text-blue-500" />
            <MetricCard title="Avg. Listen Time" value={formatTime(metrics.avgListenTime)} icon={Clock} subtitle="per session" iconColor="text-purple-500" />
            <MetricCard title="Total Likes" value={metrics.totalLikes.toLocaleString()} icon={Heart} iconColor="text-pink-500" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <MetricCard title="Uploaded Songs" value={metrics.totalSongs} icon={Music} subtitle="total tracks" iconColor="text-primary" />
            <MetricCard title="Profile Views" value={0} icon={Eye} iconColor="text-green-500" />
            <MetricCard title="Engagement Rate" value="0%" icon={TrendingUp} iconColor="text-orange-500" />
          </div>
        </TabsContent>
        ...
        <TabsContent value="songs" className="space-y-6">
          ...
          {uploadedSongs.length === 0 ? (
            <div className="text-center py-16 bg-card rounded-lg border border-border">
              ...
              <Button onClick={() => setIsUploadOpen(true)} className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full">
                <Plus className="w-4 h-4 mr-2" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {uploadedSongs.map((song) => (
                <div key={song.id} className="bg-card rounded-lg p-4 border border-border hover:bg-card/80 transition-colors group">
                  <div className="aspect-square rounded-md overflow-hidden mb-3 bg-muted">
                    {song.coverUrl ? <img src={song.coverUrl} alt={song.name} className="w-full h-full object-cover" /> : <Music className="m-auto h-full w-16 text-muted-foreground" />}
                  </div>
